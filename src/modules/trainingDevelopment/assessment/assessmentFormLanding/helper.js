...
export const assessmentColumn = (history, page, paginationSize) => {
  return [
    {
      title: "SL",
    ...
    },
    {
      ...
      render: (_, record) => (
        <div className="d-flex align-items-center ml-2">
          <div>{record?.numTotalDuration + " Hrs"}</div>
        </div>
      ),
      filter: false,
      sorter: false,
    ...
    },
    {
      ...
      render: (_, record) => (
        <div className="d-flex align-items-center ml-2">
          <div>{`${record?.strBatchNo} (${record?.intBatchSize})`}</div>
        </div>
      ),
      filter: false,
      sorter: false,
...
export const assessmentList = async (
  ...
  try {
    const res = await axios.get(
      `/Training/GetTrainingAssessmentLanding?intAccountId=${accId}&intBusinessUnitId=${buId}`
    );
    setter(res?.data);
    setAllData(res?.data);
