...
export const salaryBreakdownCreateNApply = async (payload, setLoading, cb) => {
  ...
  try {
    const res = await axios.post(
      `/Payroll/SalaryBreakdownCreateNApplyForTax`,
      payload
    );
    cb && cb();
...
export const getAllAppliedSalaryBreakdownList = async (
  ...
  try {
    const res = await axios.get(
      `/Payroll/GetAllSalaryBreakdownLandingForTax?accountId=${accId}&businessUnitId=${buId}&workplaceGroupId=${workplaceGroupId}&workplaceId=${workplaceId}&departmentId=${departmentId}&designationId=${designationId}&employmentTypeId=${employeeTypeId}&EmployeeId=${employeeId}`
    );
    if (res?.data) {
      setter(res?.data);
      setLoading && setLoading(false);
