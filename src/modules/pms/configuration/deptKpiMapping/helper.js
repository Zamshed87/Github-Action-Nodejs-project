...
export const onGetDepartmentalKpiLanding = ({
            ...
            <button
              ...
              onClick={(e) => {
                ...
                if (fromLanding) {
                  ...
                  history?.push({
                    pathname: "/pms/configuration/deptkpimapping/edit",
                    state: {
...
export const handleCreateKpiMapping = ({
  ...
  if (!rowDto?.length) {
    return toast.warning("Please Add KPI");
  }
  let payload = (
...
export const getKPIsCreateMappingData = async ({
  ...
  try {
    const res = await axios.get(
      `/PMS/GetAllDepartmentAndSbuKPIData?KPIForId=2&accountId=${orgId}&businessUnitId=${buId}&departmentId=${deptId}`
    );
    if (res?.data) {
