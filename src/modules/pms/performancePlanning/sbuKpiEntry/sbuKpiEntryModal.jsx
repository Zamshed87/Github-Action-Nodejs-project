  ...
  useEffect(() => {
    if (currentItem) {
      getPreviousEntryData(
        `/PMS/GetTargetVsAchievementById?BusinessUnit=184&YearId=${previousLandingvalues?.year?.value}&KpiForId=3&KpiForReffId=${previousLandingvalues?.sbu?.value}&objectiveId=${currentItem?.intStrategicParticularsID}&kpiId=${currentItem?.kpiId}&accountId=${intAccountId}`,
        (data) => {
          setTargetList(data);
        }
      );
  ...
  const saveHandler = (values, cb) => {
    ...
    targetList?.forEach((item) => {
      payload.push({
        ...
        intBusinessUnitId: previousLandingvalues?.department?.value,
                ...
                <div className="row">
                  <div className="col-lg-12 pl-5 pr-5">
                    ...
                    >
                      ...
                      <b className="ml-2"> Sbu</b> :{" "}
                      {previousLandingvalues?.sbu?.label}
                    </p>
                  </div>
...
export default SbuKpiEntryModal;
