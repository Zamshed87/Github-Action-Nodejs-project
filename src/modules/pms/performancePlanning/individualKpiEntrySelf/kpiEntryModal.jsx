...
const IndividualKpiEntrySelfModal = ({
  currentItem,
  setIsShowModal,
  profileData,
  ...
  useEffect(() => {
    if (currentItem) {
      getPreviousEntryData(
        `/PMS/GetTargetVsAchievementById?BusinessUnit=184&YearId=${previousLandingvalues?.year?.value}&KpiForId=1&KpiForReffId=${profileData?.employeeId}&objectiveId=${currentItem?.intStrategicParticularsID}&kpiId=${currentItem?.kpiId}&accountId=${intAccountId}`,
        (data) => {
          setTargetList(data);
        }
      );
                ...
                <div className="row">
                  <div className="col-lg-12 pl-5 pr-5">
                    ...
                    <p
                      ...
                      className="mt-3 pb-3 employee_info"
                    >
                      <b> Name</b> : {profileData?.strDisplayName}
                      <b> Enroll</b> : {profileData?.employeeId}
                      <b> Designation</b> : {profileData?.strDesignation}
                    </p>
                  </div>
...
export default IndividualKpiEntrySelfModal;
