...
import { getAssessmentTypeDDL } from "./constants";
...
const BarAssesmentLanding = () => {
  ...
  useEffect(() => {
    getFiscalYearDDL(`/PMS/GetFiscalYearDDL`, (data) => {
      ...
      // barAssesmentInitialValues.yearDDL = currYearOption;
  ...
  useEffect(() => {
    const assessmentType = barAssesmentInitialValues?.assessmentToDDL?.value;
    const year = barAssesmentInitialValues?.yearDDL?.value;
    const quarter = barAssesmentInitialValues?.quarterDDL?.value || 0;

    //bar assessment url
    const url = `/PMS/GetBarAssesmentEmployee?AccountID=1&AssesmentType=${assessmentType}&EmployeeID=${intEmployeeId}&Year=${year}&Quater=${quarter}`;
          ...
          <div className="row py-5 py-lg-0">
            <div className="input-field-main col-lg-3">
              <FormikSelect
                classes="input-sm"
                label="Assessment To"
                name="assessmentToDDL"
                options={getAssessmentTypeDDL()}
                value={values?.assessmentToDDL}
                onChange={(valueOption) => {
                  if (valueOption) {
                    setFieldValue("assessmentToDDL", valueOption);
                    setBarAssessmentData([]);
                  } else {
                    setFieldValue("assessmentToDDL", "");
                    setBarAssessmentData([]);
                  }
                }}
                styles={customStyles}
...
export default BarAssesmentLanding;
