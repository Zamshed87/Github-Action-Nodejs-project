...
export const createEmploymentType = async (payload, setLoading, cb) => {
  setLoading && setLoading(true);
  try {
    const res = await axios.post(`/SaasMasterData/SaveEmploymentType`, payload);
    cb();
...
export const getGlobalEmploymentTypeById = async (setter, id, setLoading) => {
  setLoading && setLoading(true);

  // let status = statusId ? `&intStatusId=${statusId}` : "";
  try {
    const res =
      await axios.get(`/SaasMasterData/GetEmpEmploymentTypeById?id=${id}
`);
    if (res?.data) {
...
export const adminEmpTypeDtoCol = (permission, setOpen, setId) => {
  return [
    {
      title: "SL",
