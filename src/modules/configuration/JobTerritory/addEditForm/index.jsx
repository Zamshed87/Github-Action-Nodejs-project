...
export default function AddEditForm({
  ...
  const saveJobTerritory = useApiRequest({});
  ...
  const submitHandler = ({ values, resetForm, setIsAddEditForm }) => {
    ...
    let payload = {
      ...
      workplaceGroupId: wgId,
      workplaceId: wId,
      name: values?.jobTerritory,
    };

    saveJobTerritory.action({
      urlKey: "JobTerritory",
        ...
        <Row gutter={[10, 2]}>
          <Col md={12} sm={24}>
            <PInput
              type="text"
              name="jobTerritory"
              label="Job Territory Name"
              placeholder=""
              rules={[{ required: true, message: "Job Territory is required" }]}
            />
          </Col>
