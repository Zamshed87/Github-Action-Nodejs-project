...
const AttendanceCalenderComp = () => {
  ...
  useEffect(() => {
    const absent = attendanceListDate.filter(
      (itm) => itm?.presentStatus.toLowerCase() === "absent"
    );
    const leave = attendanceListDate.filter(
      (itm) => itm?.presentStatus.toLowerCase() === "leave"
    );
    const late = attendanceListDate.filter(
      (itm) => itm?.presentStatus.toLowerCase() === "late"
    );
    const present = attendanceListDate.filter(
      (itm) => itm?.presentStatus.toLowerCase() === "present"
    );
    const movement = attendanceListDate.filter(
      (itm) => itm?.presentStatus.toLowerCase() === "movement"
    );
    setTotalAbsent(absent.length);
    setTotalLeave(leave.length);
    setTotalLate(late.length);
    setTotalPresent(present.length);
    setTotalMovement(movement.length);
    setTotalWorkingDays(present.length + late.length);
  }, [attendanceListDate]);
