...
const TransferHistory = forwardRef((props: any, ref: any) => {
  const historyData = props?.historyData;

  const transferData = historyData?.filter(
    (dto: any) => dto?.strTransferNpromotionType === "Transfer"
  );
      ...
      {transferData?.length > 0 ? (
        <div>
          <table className="table table-bordered mt-3">
            ...
            <tbody>
              {transferData?.map((item: any, index: number) => (
                <tr
                  style={{
