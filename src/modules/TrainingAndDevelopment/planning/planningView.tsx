...
import { calculateDuration, calculateTotalDuration } from "./helper";

const valueStyle = { fontSize: "14px", fontWeight: "550" };
const labelStyle = { fontSize: "12px" };
const PlanningView = ({ data, dataDetails, scheduleDetails }: any) => {
  const [scheduleDetailState, setScheduleDetails] = useState<any>([]);
  const headerCost = [
    {
      title: "SL",
  ...
  const columnsTime = [
    ...
    },
    {
      ...
      render: (trainingStartDate: any) => dateFormatter(trainingStartDate),
    },
    {
      title: "Start Time",
  ...
  useEffect(() => {
    setScheduleDetails(
      scheduleDetails?.map((item: any) => ({
        id: item.id,
        trainingStartTime: moment(item.startTime, "HH:mm:ss").format(
          "hh:mm:ss A"
        ),
        trainingEndTime: moment(item.endTime, "HH:mm:ss").format("hh:mm:ss A"),
    ...
    <>
      <Row gutter={[10, 2]} style={{ paddingLeft: "15px" }}>
        ...
        <Col md={8} style={{ marginTop: "10px" }}>
          ...
          <div style={valueStyle}>
            {data?.trainingModeStatus?.label || "N/A"}
          </div>
        </Col>
        <Col md={8} style={{ marginTop: "10px" }}>
      ...
      <Row>
        ...
        {scheduleDetails?.length > 0 && (
          <>
            <h1>
              List of Training Schedule, Duration:{" "}
              {calculateTotalDuration(scheduleDetailState)}
            </h1>
            <div className="mb-3 mt-2">
              <DataTable
                bordered
                data={scheduleDetailState || []}
                loading={false}
                header={columnsTime}
              />
            </div>
