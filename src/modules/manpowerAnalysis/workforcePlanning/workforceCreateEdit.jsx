...
const WorkForceCreate = () => {
  ...
  useEffect(() => {
    ...
    const fetchEditData = async (workplaceId, yearTypeId, fromYear) => {
      try {
        setLoading(true);
        const res = await axios.get(
          `/WorkforcePlanning/GetById?WorkplaceId=${workplaceId}&YearTypeId=${yearTypeId}&FromDate=${fromYear}`
        );
        if (res.data?.statusCode === 200 && res.data?.data) {
          ...
          form.setFieldsValue({
            ...
            selectYear: { value: d.fromDate, label: d.fromDate?.toString() },
            planningType: {
          ...
          // Set table data
          setTableData(
            (d.rowData || []).map((item, idx) => ({
              id: item.typeId,
              headerId: item.headerId, // <-- preserve headerId from API
              sl: idx + 1,
    ...
    if (location.state) {
      const { workplaceId, yearTypeId, fromYear } = location.state;
      if (workplaceId && yearTypeId && fromYear) {
        fetchEditData(workplaceId, yearTypeId, fromYear);
      }
    }
