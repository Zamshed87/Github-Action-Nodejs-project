...
import BlockedEmp from "./components/BlockedEmp";
...
const EmOvertimeAutoGenerated = () => {
  ...
  const [open, setOpen] = useState({ isActive: false, blockedEmp: [] });
  ...
  const rowDtoHandler = (name, index, value, currentValue, maxOverTime) => {
    // name = "numMinutes"
    const data = [...overTimeEmpList];
    data[index][name] = value;

    try {
      if (maxOverTime < +value * 60) {
        data[index][name] = currentValue;
        return toast.warn(
          `Daily Overtime can't be more than ${maxOverTime / 60} Hours`
        );
      }
  ...
  // save
  const saveFromFieldHandler = (isSkip = false) => {
    // setLoading(true);
    const callBack = () => {
    ...
    const payload = overTimeEmpList
      ...
      .map((item) => {
        return {
          ...
          employeeCheck: {
            intOTHourShouldBeAboveInMin: item?.intOTHourShouldBeAboveInMin,
            isManualOvertime: item?.isManualOvertime,
            strEmployeeName: item?.strEmployeeName,
            strEmployeeCode: item?.strEmployeeCode,
          },
        };
      });
    const misMatchData = payload?.filter(
      (item) =>
        !item?.employeeCheck?.isManualOvertime &&
        +item?.actualPayload?.minutes <
          +item?.employeeCheck?.intOTHourShouldBeAboveInMin
    );
    console.log(misMatchData);
    if (misMatchData?.length > 0 && !isSkip) {
      setOpen({ isActive: true, blockedEmp: misMatchData });
    } else {
      updateOvertimeHour(
        payload.map((item) => item.actualPayload),
        setLoading,
        callBack
      );
    }
  ...
  return (
    <div className="table-card">
              ...
              <div className="d-flex justify-content-end mr-2">
                <div className="row">
                  ...
                  <PrimaryButton
                    ...
                    label={"Save"}
                    onClick={() => saveFromFieldHandler(false)}
                  />
                </div>
              </div>
            </div>
                ...
                <div className="table-card-styled employee-table-card table-responsive ant-scrolling-Table">
                  ...
                  <div className="mb-3">
                    <DataTable
                      bordered
                      data={overTimeEmpList || []}
                      loading={loadingEmpList}
                      header={overTimeGeneratedDtoCol(
                        rowDtoHandler,
                        overTimeEmpList,
                        setOverTimeEmpList
                      )}
                      ...
                      pagination={{
                        ...
                        total: overTimeEmpList?.length,
                      }}
                      // filterData={landingApi?.data?.filters}
                      onChange={(pagination, filters, newRowDto) => {
      ...
      <PModal
        ...
        components={
          <BlockedEmp
            data={open.blockedEmp}
            setOpen={setOpen}
            saveFromFieldHandler={saveFromFieldHandler}
          />
        }
        width={500}
      />
