...
// import { getEmployeeLeaveBalanceAndHistoryWithFilter } from "../helper";
...
const PopOverFilter = ({ propsObj, masterFilterHandler }) => {
  ...
  useEffect(() => {
    ...
    // getPeopleDeskAllDDL = async (ddlType, accId, busId, setter, value, label, id)
    getPeopleDeskAllDDL(
      "LeaveType",
      employeeId,
      buId,
  ...
  const saveHandler = (values, cb) => {
    ...
    // getEmployeeLeaveBalanceAndHistoryWithFilter(values?.employee?.value ? values?.employee?.value : employeeId, "LeaveHistory", values?.leaveType?.value || "", values?.applicationDate || "", values?.fromDate || "", values?.toDate || "", values?.status?.value || "", setLeaveHistoryData, setLoading, "");
    setIsFilter(true);
    cb();
    handleClose();
  };
