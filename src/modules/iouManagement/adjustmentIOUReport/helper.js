...
export const getAllIOULanding = async (
  ...
  let apiUrl = `/Employee/IOULandingForAccounts?businessUnitId=${buId}&workplaceGroupId=${wgId}&fromDate=${fromDate}&toDate=${toDate}&pageNo=${pageNo}&pageSize=${pageSize}`;
  ...
  try {
    ...
    if (res?.data) {
      if (partType === "IOULandingForAccounts") {
...
export const iouAdjustmentLandingTableColumn = (
  page,
  paginationSize,
  history
) => {
                ...
                <button className="iconButton" type="button">
                  <EditOutlined
                    onClick={(e) => {
                      e.stopPropagation();
                      history?.push(
                        `/profile/iOU/adjustmentReport/${record?.iouId}`
                      );
                    }}
                  />
                </button>
