...
const SalaryCertificateApproval = () => {
  ...
  //     let newDta = allData?.listData?.filter((item) => regex.test(item?.strEmployee?.toLowerCase()));
  ...
  const demoPopup = (action, text, array) => {
    ...
    let confirmObject = {
      ...
      yesAlertFunc: () => {
        if (array.length) {
          salaryCertificateApproveReject(newArray, callback);
        }
        newArray = [];
      },
      noAlertFunc: () => {
...
export default SalaryCertificateApproval;
