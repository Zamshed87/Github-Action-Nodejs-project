...
import { getFixedRosterLanding } from "./helper";
...
export default function FixedRoster() {
  ...
  const getData = () => {
    getFixedRosterLanding(orgId, buId, setRowDto, setAllData, setLoading);
  ...
  const filterData = (keywords, allData, setRowDto) => {
    try {
      ...
      let newDta = allData?.filter((item) =>
        regex.test(item?.strRoasterName?.toLowerCase())
      );
      setRowDto(newDta);
    } catch {
      setRowDto([]);
                      ...
                      <li>
                        <PrimaryButton
                          ...
                          onClick={(e) => {
                            ...
                            history.push(
                              `/administration/timeManagement/fixedRosterSetup/create`
                            );
                          }}
                        />
                      </li>
                          ...
                          <AntTable
                            ...
                            onRowClick={(dataRow) => {
                              if (!permission?.isEdit)
                                return toast.warn("You don't have permission");
                              history.push({
                                pathname: `/administration/timeManagement/fixedRosterSetup/edit/${dataRow?.intId}`,
                                state: dataRow?.strRoasterName,
                              });
                            }}
                          />
                        </>
