                  ...
                  return (
                    <React.Fragment key={header.title}>
                      {header?.details?.map((detail) => (
                        <Table.Summary.Cell
                          key={`${header.title}-${detail}`}
                          align="center"
                        >
                          {reportData?.total?.monthlyData
                            .find((x) => x.title === header.title)
                            ?.details.find((x) => x.title === detail)?.amount ??
                            "-"}
                        </Table.Summary.Cell>
                      ))}
                      <Table.Summary.Cell
                        key={`${header.title}-total`}
                        align="center"
                      >
                        {reportData?.total?.monthlyData.find(
                          (x) => x.title === header.title
                        )?.totalAmount ?? "-"}
                      </Table.Summary.Cell>
                    </React.Fragment>
                  );
